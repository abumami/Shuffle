...
//int32_t set_options(int argc, char *argv[]) {
int32_t set_options(char *incl) {
    /*
     parses command line options
     */
    ...
    if (incl != NULL) {
        if (addDirPathToStringList(OPT_INCL_DIRS, (const char(*)[MAX_PATH_LEN+1]) incl)) {
            myerror("Could not add directory path to dirPathList");
            freeOptions();
            return -1;
        }
...
int32_t _set_options(char sort, char *incl) {
